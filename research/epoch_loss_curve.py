...
CHANNEL COUNT is a command-line number (default 8): the top-N electrodes by firing rate.
    py research/epoch_loss_curve.py         # 8 channels
    py research/epoch_loss_curve.py 32      # 32 channels
    py research/epoch_loss_curve.py 16      # 16, etc.
...
To only redraw a figure from a cached JSON (no retraining):
    py research/plot_epoch_curve.py   (or import plot_epoch_curve.render with a json/png path)
"""
...
def run(chans, out_json, out_png, title):
    ...
    try:
        import research.plot_epoch_curve as P
