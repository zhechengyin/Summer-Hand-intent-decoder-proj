...
Each session's `cubeai/` directory contains the official X-CUBE-AI 10.2
encoder and GRU/head weight binaries, a firmware-loadable `.aibundle`, and a
manifest with component and bundle size, CRC32, SHA256, graph ABI ID and
PyTorch/host-C parity results. `cubeai_bundles.json` indexes all six bundles.

The reproducible build, independent verifier, complete binary layout and graph
ABI contract are documented in `deploy/SESSION_CUBEAI_BUNDLES.md`.
