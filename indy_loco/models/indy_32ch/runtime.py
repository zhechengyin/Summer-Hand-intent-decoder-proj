...
from indy_loco.models.indy_32ch.decoder_state_detector import (
...
from indy_loco.models.indy_32ch.model import build_net

ROOT = Path(__file__).resolve().parents[2]

CHECKPOINT_PATH = ROOT / "models" / "indy_32ch" / "64x64checkpoint.pt"
DETECTOR_DIR = ROOT / "history" / "results" / "indy" / "phase3c_decoder_state_detector"
    ...
    def load(
        ...
        *,
        ...
        if feature_std_floor.shape != (64, 1):
            raise ValueError(
                "Checkpoint feature variance floor must have shape (64, 1)"
            )
        ...
        expected_bins = int(
            round(checkpoint["observation_seconds"] / gate.layer1.config.bin_seconds)
        )
        ...
        *,
        ...
        with torch.inference_mode():
            for start in range(0, len(windows), self.batch_size):
                batch = torch.from_numpy(windows[start : start + self.batch_size]).to(
                    self.device
                )
        ...
        bin_end_time = (
            np.arange(first_bin, first_bin + len(prediction), dtype=np.float64) + 1
        ) * self.bin_seconds
