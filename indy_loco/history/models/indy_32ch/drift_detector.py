...
class DriftDetector:
    ...
    def _prefixes(self, sessions: Mapping[str, np.ndarray]) -> dict[str, np.ndarray]:
        names = tuple(sorted(sessions))
        assert_pre_january(names)
        if len(names) < 3:
            raise ValueError("At least three reference sessions are required")
        return {name: _validate_counts(sessions[name], self.config) for name in names}
    ...
    def _fit_rate_references(
        ...
        for month in sorted({session_month(name) for name in rates}):
            ...
            active_masks[month] = (
                month_rates >= self.config.expected_active_rate_hz
            ).mean(axis=0) >= self.config.expected_active_fraction
    ...
    def _simple_metrics(
        ...
        unexpected_silent = int(
            np.sum((rate <= self.config.silent_rate_hz) & active_masks[matched])
        )
        return (
            matched,
    ...
    def _nested_month_calibration(
        ...
        for held_month in months:
            ...
            rows.extend(
                inner_detector._raw_metrics(values) for values in inner_held.values()
            )
        return rows
    ...
    def score(self, counts: np.ndarray) -> DetectorScore:
        ...
        global_mindful_abnormal = (
            float(metrics["global_mindful_kld"]) > self.thresholds["global_mindful_kld"]
        )
        ...
        simple_count = (
            int(pattern_abnormal) + int(global_rate_abnormal) + int(dropout_abnormal)
        )
        ...
        return DetectorScore(
            ...
            unexpected_silent_channels=int(metrics["unexpected_silent_channels"]),
            global_mindful_kld=float(metrics["global_mindful_kld"]),
            multi_reference_mindful_kld=float(metrics["multi_reference_mindful_kld"]),
    ...
    def load(cls, path: str | Path) -> DriftDetector:
        ...
        with np.load(artifact, allow_pickle=False) as arrays:
            ...
            if months != detector.reference_months:
                raise ValueError("Layer-1 month arrays do not match artifact metadata")
            ...
            rate_profiles = np.asarray(arrays["month_rate_profiles"], dtype=np.float64)
            ...
            detector.month_gaussians = {
                month: GaussianSummary(month_means[index], month_covariances[index])
