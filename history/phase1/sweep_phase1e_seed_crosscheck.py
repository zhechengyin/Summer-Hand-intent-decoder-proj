#!/usr/bin/env python
"""Phase-1e seed cross-check for the Indy 32-channel decoder.

Phase-1d left weight decay {0.025, 0.060} practically tied across seeds
{42, 43, 44}. This independent cross-check fixes learning rate at 9e-4 and
pre-GRU dropout at 0.025, then compares the same two candidates on seeds
{45, 46}. The six matching seed-42/43/44 cells are read from the completed
Phase-1d metrics JSON for aggregate reporting; they are not retrained.
...
import earlier phase scripts, ``common.py``, or anything under ``history/``.
January test session names are recorded as locked, but their arrays are never
loaded. The final five-seed recommendation follows a predeclared practical
stopping rule rather than selecting the numerically best mean alone.
"""
...
CROSSCHECK_SEEDS = (45, 46)
ALL_SEEDS = (*REFERENCE_SEEDS, *CROSSCHECK_SEEDS)
WEIGHT_DECAYS = (0.025, 0.060)
GRID_SPACE = {
    "seed": list(CROSSCHECK_SEEDS),
    "weight_decay": list(WEIGHT_DECAYS),
}
GRID_SIZE = len(CROSSCHECK_SEEDS) * len(WEIGHT_DECAYS)

MIN_SEED_WINS = 3
MIN_MEAN_LOSS_ADVANTAGE = 0.003
MAX_GUARDRAIL_R2_DROP = 0.005
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate data, Phase-1d references, model, and grid without training.",
    )
    parser.add_argument(
        "--device",
...
def load_phase1d_references() -> list[dict]:
    """Load the six comparable seed-42/43/44 cells from Phase-1d evidence."""
    if not PHASE1D_METRICS_PATH.exists():
        raise FileNotFoundError(
            "Completed Phase-1d metrics are required for Phase-1e: "
            f"{PHASE1D_METRICS_PATH}"
        )
    payload = json.loads(PHASE1D_METRICS_PATH.read_text(encoding="utf-8"))
    ...
    references = []
    rows = payload.get("all_seed_rows", [])
    for seed in REFERENCE_SEEDS:
        for weight_decay in WEIGHT_DECAYS:
            matches = [
                ...
                and math.isclose(
                    float(row.get("weight_decay", -1)), weight_decay
                )
                and math.isclose(float(row.get("dropout", -1)), FIXED_DROPOUT)
            ]
            if len(matches) != 1:
                raise ValueError(
                    "Expected one Phase-1d reference cell for "
                    f"seed={seed}, weight_decay={weight_decay}; found {len(matches)}"
                )
            row = matches[0]
            required = (
                "validation_loss",
            ...
            if missing:
                raise ValueError(
                    f"Phase-1d reference seed={seed}, wd={weight_decay} lacks {missing}"
                )
            references.append(
                {
                    **row,
                    "source": "phase1d_reused_not_retrained",
                    "seed": seed,
...
def crosscheck_rows(study, phase1d_references: list[dict]) -> list[dict]:
    """Return comparable flat rows from Phase-1d and new Phase-1e trials."""
    rows = [dict(row) for row in phase1d_references]
    for trial in completed_trials(study):
        rows.append(
            {
                "source": "phase1e_new_training",
...
def aggregate_candidates(rows: list[dict]) -> dict:
    ...
    seed_wins = {
        f"weight_decay_{weight_decay:.3f}": 0 for weight_decay in WEIGHT_DECAYS
    }
    seed_wins["exact_ties"] = 0
    for seed in expected_seeds:
        ...
        if math.isclose(loss_delta, 0.0, abs_tol=1e-12):
            seed_wins["exact_ties"] += 1
        else:
    ...
    mean_loss_winner = None
    decision = {
        "status": "incomplete",
        "recommended_weight_decay": None,
        "tie_break_weight_decay": TIEBREAK_WEIGHT_DECAY,
        "criteria": {
            "minimum_seed_wins": MIN_SEED_WINS,
            "minimum_mean_validation_loss_advantage": MIN_MEAN_LOSS_ADVANTAGE,
            "maximum_allowed_macro_r2_drop": MAX_GUARDRAIL_R2_DROP,
            "maximum_allowed_worst_session_r2_drop": MAX_GUARDRAIL_R2_DROP,
        },
    }
    if complete:
        winner_summary = min(
        ...
        mean_loss_winner = float(winner_summary["weight_decay"])
        other_summary = next(
            item
            for item in summaries
            if not math.isclose(float(item["weight_decay"]), mean_loss_winner)
        )
        loss_advantage = float(
            other_summary["metrics"]["validation_loss"]["mean"]
            - winner_summary["metrics"]["validation_loss"]["mean"]
        )
        macro_delta = float(
            winner_summary["metrics"]["validation_macro_r2_mean"]["mean"]
            - other_summary["metrics"]["validation_macro_r2_mean"]["mean"]
        )
        worst_delta = float(
            winner_summary["metrics"][
                "validation_worst_session_r2_mean"
            ]["mean"]
            - other_summary["metrics"][
                "validation_worst_session_r2_mean"
            ]["mean"]
        )
        winner_key = f"weight_decay_{mean_loss_winner:.3f}"
        criteria_passed = {
            "seed_wins": seed_wins[winner_key] >= MIN_SEED_WINS,
            "mean_loss_advantage": loss_advantage >= MIN_MEAN_LOSS_ADVANTAGE,
            "macro_r2_guardrail": macro_delta >= -MAX_GUARDRAIL_R2_DROP,
            "worst_session_r2_guardrail": worst_delta >= -MAX_GUARDRAIL_R2_DROP,
        }
        decisive = all(criteria_passed.values())
        decision = {
            **decision,
            "status": "selected_by_preregistered_rule" if decisive else "practical_tie",
            "mean_loss_winner": mean_loss_winner,
            "mean_validation_loss_advantage": loss_advantage,
            "winner_seed_wins": seed_wins[winner_key],
            "winner_macro_r2_delta": macro_delta,
            "winner_worst_session_r2_delta": worst_delta,
            "criteria_passed": criteria_passed,
            "recommended_weight_decay": (
                mean_loss_winner if decisive else TIEBREAK_WEIGHT_DECAY
            ),
            "recommendation_reason": (
                "candidate passed every preregistered criterion"
                if decisive
                else "candidates are practically tied; use the predeclared tiebreak"
            ),
        }
    return {
        "complete": complete,
...
def write_metrics(
    ...
    payload = {
        ...
        "direction": "minimize",
        "primary_metric": "mean_pooled_validation_normalized_mse_across_seeds_42_to_46",
        "fixed_learning_rate": FIXED_LEARNING_RATE,
        "fixed_dropout": FIXED_DROPOUT,
        "reference_seeds": list(REFERENCE_SEEDS),
        "new_training_seeds": list(CROSSCHECK_SEEDS),
        "weight_decay_candidates": list(WEIGHT_DECAYS),
        ...
        "phase1d_reference": {
            "source": report_path(PHASE1D_METRICS_PATH),
            "policy": "reused_metrics_not_retrained",
            "cells": phase1d_references,
        },
...
def plot_crosscheck(study, phase1d_references: list[dict]) -> None:
    rows = crosscheck_rows(study, phase1d_references)
    if not rows:
        return
    ...
    figure.suptitle("Indy 32-channel Phase-1e five-seed cross-check")
    figure.text(
        0.5,
        0.01,
        "lr=9e-4; dropout=0.025; session-balanced; seeds 42-44 reused from "
        "Phase-1d; seeds 45-46 newly trained; January test locked.",
        ha="center",
        color="#5B6470",
        fontsize=9,
    )
...
def main() -> None:
    ...
    phase1d_references = load_phase1d_references()
    ...
    print("=== Indy 32-channel Phase-1e seed cross-check ===")
    print(
    ...
    print(
        ...
        f"seeds={list(CROSSCHECK_SEEDS)} | device={device} | sampler=session-balanced"
    )
    print(
    ...
    print("seeds 42/43/44: reused from Phase-1d metrics; not retrained")
    print(
        "stop rule: >=3/5 seed wins; mean loss advantage >=0.003; "
        "macro/worst R2 drop <=0.005; otherwise tie -> wd=0.025"
    )
    ...
    torch.manual_seed(CROSSCHECK_SEEDS[0])
    ...
    for reference in phase1d_references:
        print(
            "Phase-1d reference | "
    ...
    protocol_signature = {
        ...
        "device": str(device),
        "phase1d_reference": report_path(PHASE1D_METRICS_PATH),
        "stop_rule": {
            "minimum_seed_wins": MIN_SEED_WINS,
            "minimum_mean_validation_loss_advantage": MIN_MEAN_LOSS_ADVANTAGE,
            "maximum_guardrail_r2_drop": MAX_GUARDRAIL_R2_DROP,
            "practical_tie_weight_decay": TIEBREAK_WEIGHT_DECAY,
        },
    }
    sampler = optuna.samplers.GridSampler(GRID_SPACE, seed=REFERENCE_SEEDS[0])
    ...
    if saved_signature is not None and saved_signature != protocol_signature:
        raise RuntimeError(
            "The existing Phase-1e study uses a different protocol. Choose a new "
    ...
    study.set_user_attr(
        "phase1d_policy", "reused_seed42_43_44_metrics_not_retrained"
    )
    ...
    context = {
        ...
        "fixed_protocol": {
            **protocol_signature,
            ...
            "aggregate_selection": "preregistered_five_seed_practical_stop_rule",
            "pruning": "disabled",
        },
        "split": {
    ...
    def objective(trial) -> float:
        seed = int(trial.suggest_categorical("seed", CROSSCHECK_SEEDS))
        weight_decay = float(
    ...
    def persist_callback(current_study, _trial) -> None:
        write_metrics(current_study, context, phase1d_references, started_at)
        plot_crosscheck(current_study, phase1d_references)
    ...
    if remaining:
        ...
        try:
            ...
            write_metrics(study, context, phase1d_references, started_at)
            plot_crosscheck(study, phase1d_references)
    else:
        write_metrics(study, context, phase1d_references, started_at)
        plot_crosscheck(study, phase1d_references)
    ...
    rows = crosscheck_rows(study, phase1d_references)
    aggregate = aggregate_candidates(rows)
    print("\n=== Phase-1e five-seed validation summary ===")
    ...
    if aggregate["complete"]:
        decision = aggregate["stop_rule_decision"]
        print(
            "mean-loss winner: "
            f"weight_decay={aggregate['mean_loss_winner']:.3f} | "
            f"advantage={decision['mean_validation_loss_advantage']:.5f}"
        )
        print(
            "seed wins: "
            + ", ".join(
        ...
        print(
            f"stop-rule status: {decision['status']} | "
            f"recommended weight_decay={decision['recommended_weight_decay']:.3f}"
        )
        print(
            "criteria: "
            + ", ".join(
                f"{name}={'PASS' if passed else 'FAIL'}"
                for name, passed in decision["criteria_passed"].items()
            )
        )
    else:
