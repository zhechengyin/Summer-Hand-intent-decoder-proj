...
from models.indy_32ch.drift_detector import (
    DetectorScore,
    DriftDetector,
    assert_pre_january,
    session_month,
...
from models.indy_32ch.input_pipeline import apply_feature_stats, fit_feature_stats
...
def _validate_trace(
    ...
    if output.ndim != 2 or output.shape[1] != 2:
        raise ValueError(f"Predicted velocity must have shape (bins, 2), got {output.shape}")
...
def extract_decoder_prefix_trace(
    ...
    if counts.ndim != 2 or counts.shape[0] != 32:
        raise ValueError(f"Expected selected counts with shape (32, bins), got {counts.shape}")
    ...
    prediction = (
        prediction_norm[0].cpu().numpy()
        * np.asarray(target_std, dtype=np.float32)
...
def _regularized_gaussian(
    ...
    if values.ndim != 2 or values.shape[0] < 2:
        raise ValueError(f"Gaussian samples must be a non-trivial 2D array: {values.shape}")
...
def _symmetric_kld(left: GaussianSummary, right: GaussianSummary) -> float:
    ...
    def directional(reference: GaussianSummary, current: GaussianSummary) -> float:
        ...
        solved_covariance = np.linalg.solve(
            current.covariance, reference.covariance
        )
...
class DecoderStateDetector:
    ...
    def __init__(self, config: DecoderStateConfig = DecoderStateConfig()) -> None:
        self.config = config
    ...
    def _fit_hidden_projection(
        ...
        raw = np.concatenate(
            [trace.hidden_states for trace in traces.values()], axis=0
        )
    ...
    def _project_hidden(self, hidden: np.ndarray) -> np.ndarray:
        ...
        return ((hidden - self.hidden_mean) / self.hidden_std) @ self.hidden_components.T

    def _raw_metrics(
        self, trace: DecoderPrefixTrace
    ) -> dict[str, float | str]:
        ...
        output_gaussian = _regularized_gaussian(
            values.predicted_velocity, self.config
        )
    ...
    def score(self, trace: DecoderPrefixTrace) -> DecoderStateScore:
        ...
        severe_count = (
            int(hidden_severe) + int(output_severe) + int(temporal_severe)
        )
        ...
        gate_decision = (
            "abstain" if hidden_severe and output_state_severe else "pass"
        )
    ...
    def metadata(self) -> dict:
        return {
            ...
            "decision_policy": {
                ...
                "diagnostic_abstain": (
                    "at least two decoder-state evidence families"
                ),
                "gate_abstain": (
    ...
    def save(self, path) -> None:
        ...
        arrays = {
            ...
            "hidden_month_covariances": np.stack(
                [
                    self.hidden_month_gaussians[month].covariance
                    for month in months
                ]
            ).astype(np.float32),
            ...
            "output_month_covariances": np.stack(
                [
                    self.output_month_gaussians[month].covariance
                    for month in months
                ]
            ).astype(np.float32),
            "output_delta_month_means": np.stack(
                [
                    self.output_delta_month_gaussians[month].mean
                    for month in months
                ]
            ).astype(np.float32),
    ...
    def load(cls, path: str | Path) -> "DecoderStateDetector":
        ...
        with np.load(artifact, allow_pickle=False) as arrays:
            ...
            if months != detector.reference_months:
                raise ValueError(
                    "Layer-2 month arrays do not match artifact metadata"
                )
            ...
            def summaries(mean_key: str, covariance_key: str):
                ...
                covariances = np.asarray(
                    arrays[covariance_key], dtype=np.float64
                )
